...
gaeguli_pipeline_finalize (GObject * object)
{
  ...
  g_clear_pointer (&self->device_monitor, gst_object_unref);
  g_clear_pointer (&self->loop, g_object_unref);
  g_clear_handle_id (&self->benchmark_timeout_id, g_source_remove);
...
static gchar *
_get_stream_props_description ()
{
  GString *stream_props = g_string_new (PIPEWIRE_NODE_STREAM_PROPERTIES_STR);

  g_string_append_printf (stream_props, ",%s=%s-%d",
      PIPEWIRE_NODE_NAME_STR, GAEGULI_PIPELINE_TAG, getpid ());
...
_get_vsrc_pipeline_string (GaeguliPipeline * self)
{
  ...
  g_autofree gchar *props = _get_stream_props_description ();
...
_is_target_device (GstDevice * target_device)
{
  ...
  if (g_strrstr (device_class, PIPEWIRE_DEVICE_CLASS_STR)) {
    res = TRUE;
  }
  g_free (device_class);
...
static guint
_get_target_nodeid (GstDevice * target_device, gchar * device_id)
{
  GstStructure *props;
  guint node_id = 0;
  ...
  if (props) {
    const gchar *node_name, *node_id_str;

    if (gst_structure_has_field (props, PIPEWIRE_NODE_NAME_STR)
        && gst_structure_has_field (props, PIPEWIRE_NODE_ID_STR)) {
      node_name = gst_structure_get_string (props, PIPEWIRE_NODE_NAME_STR);
      if (!g_strcmp0 (node_name, device_id)) {
...
static gchar *
_get_pipeline_id_description ()
{
  GString *desc = g_string_new (NULL);

  g_string_append_printf (desc, "%s-%d", GAEGULI_PIPELINE_TAG, getpid ());
  ...
  switch (GST_MESSAGE_TYPE (message)) {
    case GST_MESSAGE_DEVICE_ADDED:{
      ...
      if (_is_target_device (device)) {
        g_autofree gchar *pipeline_id = _get_pipeline_id_description ();
        self->target_node_id = _get_target_nodeid (device, pipeline_id);
...
gaeguli_pipeline_create_device_monitor (GaeguliPipeline * self)
{
  ...
  self->device_monitor = gst_device_monitor_new ();

  bus = gst_device_monitor_get_bus (self->device_monitor);
  gst_bus_add_watch (bus, _bus_device_monitor, self);
  gst_object_unref (bus);

  caps = gst_caps_new_empty_simple ("video/x-raw");
  gst_device_monitor_add_filter (self->device_monitor, "Video", caps);
  gst_caps_unref (caps);

  self->loop = g_main_loop_new (NULL, FALSE);
  gst_device_monitor_start (self->device_monitor);
    ...
    const gchar * location, gboolean is_record_target, GError ** error)
{
  ...
  if (!target) {
    ...
    target = gaeguli_target_new (target_id, codec, bitrate,
        ...
        self->target_node_id, &internal_err);
    ...
    GError ** error)
{
  ...
  gst_device_monitor_stop (self->device_monitor);
